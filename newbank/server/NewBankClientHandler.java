...
public class NewBankClientHandler extends Thread {
	...
	public void run() {
		...
		try {
			...
			// Case of login failed, ask the user attempts to try again. Max of attempt is
			// 5.
			...
			while (attempts < 5) {
				...
				if (customerId != null) {
					...
					out.println(
							"For example, type MOVE 5000 Main Savings to move $5000 from account Main to account Savings.\n");
					Commands();
					while (true) {
						...
						String responce = bank.processRequest(customerId, request, socket);
